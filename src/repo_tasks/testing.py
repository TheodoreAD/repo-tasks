...
def _pytest(c: Context, args: str = "", *, stream: bool = False) -> None:
    """Run pytest, treating "no tests collected" as a pass.

    That tolerance is correctness rather than presentation, so it lives here and holds in both
    output modes — `warn=True` says this call site owns the exit code, and it does.

    `stream` forces the output to be shown even under `REPO_TASKS_RUN_REPORT`: `coverage` sets it
    because its output *is* the report, and the integration tiers because they run for minutes with
    a human usually watching. See `runner.py` for why an explicit `hide` is what says so."""
    ...
    # `hide=False` is not redundant: it is the marker that keeps this command streaming in report
    # mode. Dropping it folds a coverage report away. Absent, the runner decides.
    result = c.run(command, echo=True, warn=True, hide=False) if stream else c.run(command, echo=True, warn=True)
...
def _integration(c: Context, args: str, label: str) -> None:
    ...
    if not _INTEGRATION_DIR.exists():
        ...
        return
    _pytest(c, f"{_INTEGRATION_DIR} {args}".strip(), stream=True)
