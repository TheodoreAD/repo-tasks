...
def venv(c):
    ...
    if _command_exists("direnv"):
        ...
        print("[dev_env.venv] direnv not found — .venv ready, but no shell auto-activation")
...
def claude_hook(c, dir="."):  # noqa: A002
    ...
    if not envrc.exists():
        print(f"[dev_env.claude-hook] {envrc} not found — nothing to hook, skipping")
        return
    ...
    if env_ok and hook_ok:
        print(f"[dev_env.claude-hook] {settings_path} already configured")
        return
    ...
    print(f"[dev_env.claude-hook] {settings_path}: direnv hook configured ({env_file})")


@task(pre=[venv, claude_hook])
def setup(c):
    """Run once after cloning: create/refresh .venv (uv sync + direnv allow) and wire Claude
    Code's Bash tool to auto-activate it. The one command to run before anything else."""
