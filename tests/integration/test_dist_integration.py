...
def test_versions_html_fallback_round_trip(c, devpi_index):
    ...
    project = discover_python_projects(c)[0]
    normalized = dist._normalize(project.name)  # pyright: ignore[reportPrivateUsage]
    ...
    found = dist._html_versions(dist._get(url), normalized)  # pyright: ignore[reportPrivateUsage]
